...
index.ts {
  ...
  const Example = () => {
    ...
    // stateAndSetter = [ state, setState ] 　stateAndSetterに[ state, setState ]が渡る
    return (
      <MyContext.Provider value={stateAndSetter}>
        {/*　valueに{stateAndSetter}を設定　こうすることでuseContext(stateAndSetter)とすることができ、
        stateAndSetterの中身はuseState()から渡ってきた[state,setState]初期値０、となる　*/}
        <Child />
        <OtherChild />
      </MyContext.Provider>
    );
...
OtherChild.ts {
  ...
  const OtherChild = () => {
    const [, setState] = useContext(MyContext); // useContextの引数にContextオブジェクトを指定することでContext値を取得
    // MyContextに[ state, setState ]が渡ってきている　[,setState ]のみに分割代入
...
GrandChild.ts {
  ...
  import { MyContext } from "../Example";
  const GrandChild = () => {
    const [value] = useContext(MyContext); // MyContextのContext値[ state, setState ]が渡ってきているので、必要な[ value ]のみに分割代入
    return (
      <div style={{ border: "1px solid black" }}>
        <h3>孫コンポーネント</h3>
        {value}
...
export const Template: Rect.FC<TableProps,Props> = (props) => {
